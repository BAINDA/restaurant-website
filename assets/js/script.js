...
// Function to format the current date to dd/mm/yyyy
function formatCurrentDate() {
  const currentDate = new Date();
  const day = String(currentDate.getDate()).padStart(2, "0"); // Get day with leading zero
  const month = String(currentDate.getMonth() + 1).padStart(2, "0"); // Get month with leading zero
  ...
  timeElements.forEach((timeElement) => {
    ...
    timeElement.textContent = `${day}/${month}/${year}`; // Set displayed text
  });
}
